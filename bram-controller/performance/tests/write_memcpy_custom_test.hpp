...
#ifndef BRAM_CONTROLLER_PROJECT_WRITE_MEMCPY_CUSTOM_TEST_HPP
#define BRAM_CONTROLLER_PROJECT_WRITE_MEMCPY_CUSTOM_TEST_HPP
...
#include <chrono>
#include <random>
#include <commands/write/write_silent_memcpy_command.hpp>
#include <commands/write/write_silent_memcpy_custom_command.hpp>
#include "../test_statistics.hpp"

class WriteMemcpyCustomTest {
public:
    void performTest(uint address, ulong numOfTest, ulong testSize, bool saveResults, uint saveAddress, int counter) {
        printf("Running write commands with memcpy custom, %ld tests with %ld words for 0x%X\n", numOfTest, testSize, address);
        auto *writes = new double[numOfTest];
        for (int i = 0; i < numOfTest; ++i) {
            std::shared_ptr<uint> testWords = generateRandomWords(testSize);
            WriteSilentMemcpyCustomCommand writeSilentMemcpyCustomCommand(address, testSize, testWords);

            auto startWrite = std::chrono::high_resolution_clock::now();
            writeSilentMemcpyCustomCommand.execute();
...
#endif //BRAM_CONTROLLER_PROJECT_WRITE_MEMCPY_CUSTOM_TEST_HPP
