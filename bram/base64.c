...
int base64_encode(const void *data_buf, size_t data_length, char *result, size_t result_size) {
    ...
    for (x = 0; x < data_length; x += 3) {
        ...
        result[result_index++] = base64_chars[n0];
        if (result_index >= result_size) return 1;   /* indicate failure: buffer too small */
        result[result_index++] = base64_chars[n1];
        ...
        if ((x + 1) < data_length) {
            if (result_index >= result_size) return 1;   /* indicate failure: buffer too small */
            result[result_index++] = base64_chars[n2];
        }
        ...
        if ((x + 2) < data_length) {
            if (result_index >= result_size) return 1;   /* indicate failure: buffer too small */
            result[result_index++] = base64_chars[n3];
        }
    }
